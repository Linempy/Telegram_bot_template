...
from database.models import User, FileToPrepare, TestTask
...
async def insert_test_task(tast_text: str, explanation: str, 
                           true_answer: int, picture_file_id: str | None = None) -> None:
    session = create_session(engine)
    async with session.begin() as session:
        session.add(TestTask(text_of_task=tast_text, 
